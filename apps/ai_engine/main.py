...
async def dost_chat(request: Request):
    try:
        ...
        hf_response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": f"You are TruckNet Dost, a helpful AI assistant for Indian truck drivers. Reply in Hinglish in 2-3 sentences. User: {message}"
                    }
                ],
                "max_tokens": 150,
                "stream": False
        ...
        return {
            "status": "error",
            "reply": "Kuch gadbad hui, thodi der mein try karo. 🚛",
            "message_received": message
        }
